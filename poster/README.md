...
(mamahuhu.app の結果JSONは土日18時に生成されるため、その後の20時に走らせる)
...
3. 開催場ごとに `https://mamahuhu.app/data/{YYYYMMDD}_{開催場}.json` を取得
   - **土曜20時投稿** (=日曜レース): 当日(土)のJSON。当日18時生成のデータを使う
   - **金曜20時投稿** (=土曜レース): 先週日曜(5日前)のJSON
   - 404 ならその開催場はスキップ。フォールバックなし
4. 各レースの出馬表から騎手と芝/ダートを取得
5. その芝/ダートの `best_combo` (内外+脚質) からバイアス文字列を生成
6. JSON の `hot_jockeys` 全員と照合し、出走中の好調騎手をピックアップ
...
`--date YYYYMMDD` で実行日を上書きできます。曜日判定とJSON取得日もそれに合わせて変わります。
